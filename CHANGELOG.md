...
- **Migration `0012_pgr_module.sql`** — 7 tabelas alinhadas ao formulário FO-121-00:
  `pgr`, `pgr_ghe`, `pgr_ghe_cargo`, `pgr_risco`, `pgr_acao`,
...
- **Migration `0013_esocial_agente_nocivo.sql`** — catálogo global de agentes
  ...
  via service_role.
- **Seed oficial com 94 agentes** — extraído do CSV oficial
  (`TABELA22_v4_Conteudo.csv`, leiaute S-1.3). 65 químicos, 18 físicos,
...
- **Anatomia do PGR** documentada em `docs/research/pgr-anatomia.md`
  (810 linhas).
...
### Adicionado
- **Logos do produto** (SVG em `public/logos/`)
  - Horizontal claro/escuro, monocromático preto/branco, ícone principal e monocromáticos
  - Componente `<BrandLogo>` com troca automática por tema (dark/light) via Tailwind — sem flash na hidratação
...
- **Ficha de EPI cumulativa** (histórico vivo por colaborador, inspirada no SGI/FO-008-00)
